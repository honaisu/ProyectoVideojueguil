    ...
    public void render(float delta) {
        ...
        if (!paused && !jugador.estaHerido()) {
            ...
            if (asteroidManager.isEmpty()) {
            	...
            	asteroidManager = new AsteroidManager(cantAsteroides+2, velXAsteroides+2, velYAsteroides+2);
                ss.resize(1200, 800);
                game.setScreen(ss);
                dispose();
                return;
        ...
        for (Swing s: meleeManager.getSwings()) {
            ...
            s.shapeRenderer.rect(
            	    ...
            	    s.getHitbox().getSpr().getOriginY(),
            	    s.getHitbox().getSpr().getWidth(),
            	    s.getHitbox().getSpr().getHeight(),
            	    1, 1,
